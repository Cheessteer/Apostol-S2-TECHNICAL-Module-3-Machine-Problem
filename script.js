...
function animateBox(newDay) {
    ...
    box.textContent = newDay; // Display the day name
    boxCount++;
    ...
    document.body.style.backgroundColor = targetColor; // Change body bg color
    box.offsetHeight;
...
function resetAnimation() {
    document.getElementById('animated-container').innerHTML = ''; // Clear animated boxes
    document.body.style.backgroundColor = ''; // Reset body bg color
    boxCount = 0; // Reset box count
    // Reset button colors
    const buttons = document.querySelectorAll('.day-button');
    buttons.forEach(button => {
        if (!button.id.includes('reset')) { // Exclude reset button from color reset
